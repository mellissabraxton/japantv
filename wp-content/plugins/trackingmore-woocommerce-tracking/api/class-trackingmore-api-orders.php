	...
	public function get_order($id, $fields = null)
	{
		...
		if ($plugin == 'trackingmore') {
			...
			$order_data['trackingmore']['woocommerce']['trackings'][] = array(
				'tracking_provider' => get_post_meta($order->id, '_trackingmore_tracking_provider', true),
				'tracking_number' => get_post_meta($order->id, '_trackingmore_tracking_number', true),
				'tracking_ship_date' => get_post_meta($order->id, '_trackingmore_tracking_shipdate', true),
				'tracking_postal_code' => get_post_meta($order->id, '_trackingmore_tracking_postal', true),
				'tracking_account_number' => get_post_meta($order->id, '_trackingmore_tracking_account', true),
                'tracking_key' => get_post_meta($order->id, '_trackingmore_tracking_key', true),
                'tracking_destination_country' => get_post_meta($order->id, '_trackingmore_tracking_destination_country', true),
			);
		}
		if (empty($tn)) {
			$tn = get_post_meta($order->id, '_tracking_number', true);
			if ($tn == NULL) {
				if (is_array(get_post_meta($order->id, '_wc_shipment_tracking_items', true))) {
					$trackingmore_get_post_meta_number = get_post_meta($order->id, '_wc_shipment_tracking_items', true)[0]['tracking_number'];
				}else{
					$trackingmore_get_post_meta_number = '';
				}
				if (is_array(get_post_meta($order->id, '_wc_shipment_tracking_items', true))) {
					$trackingmore_get_post_meta_provider = get_post_meta($order->id, '_wc_shipment_tracking_items', true)[0]['custom_tracking_provider'];
				}else{
