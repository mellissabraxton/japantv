    ...
    class FPRewardSystemEncashTabList extends WP_List_Table {
        ...
        function extra_tablenav($which) {
            ...
            if ($which == 'top') {
                ...
                <input type="submit" class="button-primary" name="fprs_encash_export_csv_alldata" id="fprs_encash_export_csv_alldata" value="<?php _e('Export All Cashback Requests', 'rewardsystem'); ?>"/>
                <?php
                ...
                if (isset($getallresults)) {
                    foreach ($getallresults as $allvalue) {
                        if ($allvalue['pointstoencash'] != '') {
                            $mainlistarray_alldata_heading = "Username,User Current Points,Points for Cashback, Currency Code, Amount for Cashback,Reason for Encashing,Paypal Address, Other Payment Details,Application Status,Cashback Requested Date" . "\n";
