...
function rs_ajax_for_upload_your_gift_voucher($id) {
    ?>
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            var rs_custom_uploader;
            jQuery('<?php echo $id;?>').click(function (e) {
                e.preventDefault();
                ...
                rs_custom_uploader.on('select', function () {
                    ...
                    jQuery('<?php echo $id;?>').val(attachment.url);
                });
...
function rs_function_to_add_field_for_user_select($field_id, $field_label, $getuser) {
    global $woocommerce;
    ...
    <?php if ((float) $woocommerce->version <= (float) ('2.2.0')) { ?>
        <tr valign="top">
            <th class="titledesc" scope="row">
                <label for="<?php echo $field_id; ?>"><?php _e($field_label, 'rewardsystem'); ?></label>
            </th>
            <td>
                <select name="<?php echo $field_id; ?>[]" style="width:343px;" multiple="multiple" id="<?php echo $field_id; ?>" class="short <?php echo $field_id; ?>">
                    <?php
                    $json_ids = array();
                    if ($getuser != "") {
                        $listofuser = $getuser;
    ...
    <?php
        ...
        if ((float) $woocommerce->version >= (float) ('3.0.0')) {            
            ...
            <tr valign="top">
                ...
                <td>
                    <select multiple="multiple"  class="wc-customer-search"  name="<?php echo $field_id; ?>[]" id="<?php echo $field_id; ?>" data-placeholder="<?php _e('Search Users', 'rewardsystem'); ?>" >
                        <?php
                        $json_ids = array();                        
                        if ($getuser != "") {
                            $listofuser = $getuser;
            ...
            <tr valign="top">
                ...
                <td>
                    <input type="hidden" class="wc-customer-search" name="<?php echo $field_id; ?>" id="<?php echo $field_id; ?>" data-multiple="true" data-placeholder="<?php _e('Search Users', 'rewardsystem'); ?>" data-selected="<?php
                    $json_ids = array();
                    if ($getuser != "") {
                        $listofuser = $getuser;
                        if (!is_array($listofuser)) {
    ...
    <?php
        ...
        if ($woocommerce->version >= (float) ('3.0.0')) {
            ...
            <tr valign="top">
                ...
                <td class="forminp forminp-select">
                    <select class="wc-product-search" style="width: 50%;" multiple="multiple" id="<?php echo $field_id; ?>"  name="<?php echo $field_id; ?>[]" data-placeholder="<?php _e('Search for a product&hellip;', 'rewardsystem'); ?>"  >
                        <?php
                        $json_ids = array();
                    if ($getproducts != "") {
            ...
            <tr valign="top">
                ...
                <td class="forminp forminp-select">
                    <input type="hidden" class="wc-product-search" style="width: 350%;" id="<?php echo $field_id; ?>"  name="<?php echo $field_id; ?>" data-placeholder="<?php _e('Search for a product&hellip;', 'rewardsystem'); ?>" data-action="woocommerce_json_search_products_and_variations" data-multiple="true" data-selected="<?php
                    $json_ids = array();
                    if ($getproducts != "") {
