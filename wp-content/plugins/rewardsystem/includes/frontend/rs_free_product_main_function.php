...
if (!class_exists('FPRewardSystem_Free_Product')) {
    ...
    class FPRewardSystem_Free_Product {
        ...
        public static function init() {
            ...
            add_action('woocommerce_before_calculate_totals', array(__CLASS__, 'alter_free_product_price'));
        ...
        public static function fp_main_function_add_to_cart() {
            ...
            $total_earned_points = get_option('rs_select_earn_points_based_on') == '1' ? RSPointExpiry::get_sum_of_earned_points($userid) : RSPointExpiry::get_sum_of_total_earned_points($userid);
            $dynamicruleid = self::fp_get_free_product_level_id($total_earned_points);
            $getcorrespondingproducts = self::fp_get_corresponding_product($dynamicruleid);
        ...
        public static function save_data_to_order($orderid) {
            ...
            $order_user_id = rs_get_order_obj($order);
            $order_user_id = $order_user_id['order_userid'];
            $total_earned_points = get_option('rs_select_earn_points_based_on') == '1' ? RSPointExpiry::get_sum_of_earned_points($order_user_id) : RSPointExpiry::get_sum_of_total_earned_points($order_user_id);
