...
if (in_array($user_id, $emailusers)) {
    foreach ($emailusers as $myuser) {
        ...
        if (get_user_meta($myuser, 'unsub_value', true) != 'yes') {
            ...
            $earnedpointpoint = rs_common_function_to_get_earned_points_for_order($order_id);
            ...
            $earnpointsmail= $emails->earningpoints;
            if($earnpointsmail==''){
                $earnpointsmail= 0;
            }
            ...
            if ($userpoint  >= $minimumuserpoints) {
                if ($earnedpointpoint >= $earnpointsmail) {
                    ...
                    $levelname = RSMemberFunction::current_level_name($myuser, 'email');
                    ...
                    $nextlevelname = RSMemberFunction::next_earning_level_points($myuser, 'email');
