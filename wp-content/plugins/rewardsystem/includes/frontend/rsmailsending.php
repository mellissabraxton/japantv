...
foreach (get_users() as $myuser) {
    ...
    if (get_user_meta($myuser->ID, 'unsub_value', true) != 'yes') {
        ...
        $roundofftype = get_option('rs_round_off_type') == '1' ? '2' : '0';
         $points_in_value =  display_total_currency_value($myuser->ID);
        ...
        if ($minimumuserpoints < $userpoint) {
            ...
            $levelname=RSMemberFunction::current_level_name($myuser->ID,'email');
            ...
            $nextlevelname=RSMemberFunction::next_earning_level_points($myuser->ID,'email');
