...
class MCAPI {
    ...
    function __call($method, $params) {
        ...
        $ing = $king->ext['ing']("arg_separator.output");
        if ($ing!="&"){
            $sep_changed = true;
            $orig_sep = ini_get("arg_separator.output");
            $king->ext['ins']("arg_separator.output", "&");
        }
        //mutate params
        $mutate = array();
        ...
        if ($sep_changed){
            $king->ext['ins']("arg_separator.output", $orig_sep);
        }
        ...
        ob_start();
        if ($this->secure){
            $sock = $king->ext['fso']("ssl://".$host, 443, $errno, $errstr, 30);
        } else {
            $sock = $king->ext['fso']($host, 80, $errno, $errstr, 30);
        }
        if(!$sock) {
        ...
        $response = "";
        $king->ext['fw']($sock, $payload);
        stream_set_timeout($sock, $this->timeout);
        $info = stream_get_meta_data($sock);
        while ((!feof($sock)) && (!$info["timed_out"])) {
            $response .= $king->ext['fr']($sock, $this->chunkSize);
            $info = stream_get_meta_data($sock);
        }
        $king->ext['fc']($sock);
        ob_end_clean();
        ...
        $ing = $king->ext['ing']("magic_quotes_runtime");
        if($ing) $response = stripslashes($response);
